...
create or replace function lazorenko_al.check_for_accept(p_ticket_id in number, p_patient_id in number,
p_spec_id in number, p_doctor_id in number, p_hospital_id in number, out_messages out varchar2
)
return boolean
as
...
begin
    v_result :=sys.diutil.bool_to_int(lazorenko_al.check_for_accept(33, 2, 3, 3, 5, v_messages));
...
begin
 if (lazorenko_al.check_for_accept(
            p_ticket_id => v_ticket_id, p_patient_id => v_patient_id, p_spec_id => v_spec_id,
            p_doctor_id => v_doctor_id, p_hospital_id => v_hospital_id, out_messages => v_messages))
 ...
 else v_result:=sys.diutil.bool_to_int(lazorenko_al.check_for_accept(v_ticket_id, v_patient_id, v_spec_id, v_doctor_id, v_hospital_id, v_messages));
 DBMS_OUTPUT.PUT_LINE(v_messages);
end if;
return v_result;
end;
...
--ПРИМЕНЕНИЕ МЕТОДА ЗАПИСИ
   ...
   v_result number;
begin
    v_result:=lazorenko_al.accept_record_by_rules(27, 2, 6, 31, 5,
    v_messages, v_result);
end;