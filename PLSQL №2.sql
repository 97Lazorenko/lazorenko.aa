...
BEGIN
    ...
    exit when c_get_info%notfound;
    DBMS_OUTPUT.PUT_LINE('название мед.учреждения - '||v_record_beg_end.name ||': '||v_record_beg_end.day || '  ' || 'время открытия - '
                             || v_record_beg_end.begin_time || '; время закрытия - ' || v_record_beg_end.end_time);
    end loop;
    CLOSE c_get_info;
end;
