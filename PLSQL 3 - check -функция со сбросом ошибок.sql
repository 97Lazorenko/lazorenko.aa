...
create or replace function check_age(
    ...
    if v_count=0 then raise_application_error (-20400, 'возраст пациента не соответствует полу специальности');
    end if;
    return v_count>0;
    exception
    ...
    return v_count>0;
        when e_wrong_age then lazorenko_al.add_error_log(
                $$plsql_unit_owner||'.'||$$plsql_unit,
                '{"error":"' || sqlerrm
                ||'","value":"' || p_patient_id
                ||'","backtrace":"' || dbms_utility.format_error_backtrace()
...
begin
 if (lazorenko_al.check_for_accept_with_exceptions(
    ...
    p_spec_id => v_spec_id))
 then v_record_id:=lazorenko_al.pkg_write_or_cancel.write_to_records(
    p_patient_id => v_patient_id,
    p_ticket_id => v_ticket_id,
    p_need_handle => v_need_handle);

end if;
if sql%found then dbms_output.put_line(v_record_id ||' - '||'запись осуществлена успешно');
end if;
