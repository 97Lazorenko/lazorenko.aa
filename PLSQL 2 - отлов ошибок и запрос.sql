...
create or replace function write_to_records(
    ...
    if (p_need_handle) then
            ...
            exception
            when no_patient_found_or_wrong_ticket then lazorenko_al.add_error_log(
                $$plsql_unit_owner||'.'||$$plsql_unit,
                '{"error":"' || sqlerrm
                ||'","value":"' || p_patient_id
                ||'","backtrace":"' || dbms_utility.format_error_backtrace()
...
declare
    ...
    v_record number;
begin
   v_record:=lazorenko_al.write_to_records(
    ...
    create or replace function cancel_record(
    p_ticket_id in number)
    return  number  as
    v_record_id number;
    ...
    if sql%notfound
        then raise_application_error(-20500,'указан неверный талон');
    end if;
     return v_record_id;
    exception
        when no_ticket_found then lazorenko_al.add_error_log(
                $$plsql_unit_owner||'.'||$$plsql_unit,
                '{"error":"' || sqlerrm
                ||'","value":"' || p_ticket_id
                ||'","backtrace":"' || dbms_utility.format_error_backtrace()
...
declare
    ...
    v_record number;
begin
   v_record:=lazorenko_al.cancel_record(
        p_ticket_id => v_ticket
    );
    commit;
end;
...
as
    ...
    if (p_need_handle) then
            ...
            update lazorenko_al.ticket t set t.ticket_stat_id=lazorenko_al.pkg_write_or_cancel.c_record_or_ticket_stat_second where t.ticket_id=p_ticket_id;
    ...
    pragma exception_init (no_ticket_found, -20500);
    begin
    update lazorenko_al.ticket t set t.ticket_stat_id=lazorenko_al.pkg_write_or_cancel.c_record_or_ticket_stat_first where t.ticket_id=p_ticket_id;

    update lazorenko_al.records r set r.record_stat_id=lazorenko_al.pkg_write_or_cancel.c_record_or_ticket_stat_second where r.ticket_id=p_ticket_id;
